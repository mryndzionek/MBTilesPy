...
class DBMapWriter:
    ...
    def write(self, z, x, y, data):
        self.cursor.execute("""INSERT INTO tiles (zoom_level, tile_column, tile_row, tile_data)
...
class MapTileUtil:
    ...
    __base_style = ['black', 'maroon', 'green', 'olive', 'navy', 'purple', 'teal', 'silver',
                    'gray', 'red', 'lime', 'yellow', 'blue', 'fuchsia', 'aqua', 'white']

    __layer_style = {'water': 'lightblue', 'waterway': 'lightblue', 'park': 'seagreen', 'forest': 'green',
                     'mountain_peak': 'brown', 'landcover': 'gray', 'building': 'darkgray', 'poi': 'red',
                     'aeroway': 'lightgray', 'boundary': 'darkblue', 'path': 'black', 'secondary': 'darkgray',
                     'transit': 'yellow', 'residential': 'lightgray', 'industrial': 'cadetblue',
                     'cemetery': 'slategrey', 'military': 'olivedrab', 'commercial': 'slateblue',
                     'hospital': 'slateblue', 'motorway': 'black', 'grass': 'lightgreen', 'retail': 'gray',
                     'farmland': 'olive', 'allotments': 'olive', 'glacier': 'white', 'grassland': 'olive',
                     'meadow': 'olive', 'wood': 'brown', 'orchard': 'palegreen', 'wetland': 'brown',
                     'railway': 'midnightblue', 'rail': 'midnightblue', 'recreation_ground': 'mediumaquamarine',
                     'stadium': 'purple', 'school': 'navy', 'kindergarten': 'navy', 'university': 'navy',
                     'college': 'navy', 'library': 'navy', 'track': 'darkgray',
                     'primary': 'black', 'tertiary': 'black', 'trunk': 'brown',
                     'transportation_name': 'black', 'minor': 'black', 'service': 'black'}
    ...
    def get_tile_zxy(self, z, x, y):
        ...
        self.cursor.execute(
            "SELECT * FROM tiles WHERE zoom_level='{}' AND tile_column='{}' AND tile_row='{}'".format(z, x, y))
    ...
    def __poly_winding(poly):
        return 0 > sum([(x2 - x1) * (y2 - y1) for ((x1, y1), (x2, y2)) in zip(poly, poly[1:])])

    @staticmethod
    def __render(img, tp, geom, style, ex, off_x, off_y, scale_x, scale_y, name=None):
        def sp(xy): return (int((xy[0]) * scale_x), int((xy[1]) * scale_y))
        c, w = style
        ...
        if tp == 'polygon':
            for ps in geom:
                ...
                for p in ps:
                    ...
                    if w != wb:
                        tmp_canv.polygon(list(map(sp, p)), fill=(
                            0, 0, 0, 0), outline=(0, 0, 0, 0))
                    else:
                        tmp_canv.polygon(list(map(sp, p)),
            ...
            assert(len(geom) == 1)
            cp = sp(geom[0])
            canv.ellipse((cp[0] - 1, cp[1] - 1, cp[0] +
                         1, cp[1] + 1), fill=c, outline=c)
            if False:
        ...
        for l in vt.layers:
            ...
            for ft in l.features:
                ...
                if 'MAP_COLOR' in ft.attributes:
                    style = (MapTileUtil.__base_style[int(
                        ft.attributes['MAP_COLOR'])], 1)
                MapTileUtil.__render(img, ft.type, ft.get_geometry(), style,
                                     ex, 0, 0, scale_x, scale_y, name)
        return img
    ...
    def __is_visible(self, x, y):
        ...
        r2 = (x - (self.tile_size / 2), y - (self.tile_size / 2),
    ...
    def __find_centers(self):
        ...
        def neigh(p_, xy_): return [((p_[0] + (dx * self.tile_size),
                                      p_[1] + (dy * self.tile_size)),
                                     (xy_[0] + dx, xy_[1] - dy)) for dx, dy in
                                    [(-1, 0), (1, 0), (0, 1), (0, -1)]]
        base = ((cx, cy), (x, y))
        to_visit = set([base])
        ...
        while len(to_visit) > 0:
            ...
            if self.__is_visible(*p_[0]):
                points.add(p_)
                ns = set([n for n in neigh(*p_) if n[1][0] >=
                          0 and n[1][1] >= 0])
    ...
    def render_display(self, crosshairs=False):
        ...
        if center_tile:
            ...
            tiles = [(p[0][0], p[0][1], self.get_tile_zxy(
                self.zoom, p[1][0], p[1][1])) for p in ps]
            ...
            with Pool() as pool:
                for (x, y), tile_img in zip(xy, pool.imap(worker, tiles)):
                    disp_img.paste(
                        tile_img, (int(x - (self.tile_size / 2)),
            ...
            with gzip.GzipFile(fileobj=io.BytesIO(row[3])) as gz:
                ...
                for l in vt.layers:
                    for ft in l.features:
                        ...
                        style = MapTileUtil.__get_feature_style(
                            l.name, ft.attributes)
                        styles.add(style)
                        ...
                        if ft.type == 'polygon':
                            ...
                            try:
                                ml = max([len(p)
                                         for ps in ft.get_geometry() for p in ps])
                            except ValueError:
                                ml = 0
                                pass
                            if ml > max_pl:
    ...
    def encode(self, writer, zipped=True):
        ...
        for row in self.cursor:
            ...
            with gzip.GzipFile(fileobj=io.BytesIO(row[3])) as gz:
                ...
                for l in vt.layers:
                    for ft in l.features:
                        style = style_lut[MapTileUtil.__get_feature_style(
                            l.name, ft.attributes)]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        logging.error(
            'Three arguments needed: mbtile_file_name latitude longitude')
        sys.exit(1)
    ...
    img = MapTileUtil.render_tile(mbt_util.tile_size, tile)
